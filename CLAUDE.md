...
**Versione:** v0.0.7 (pre-alpha)
**Fase corrente:** Fase 1 — Fondamenta (Mesi 1-3)
**Sprint corrente:** Settimana 7 — Sessione 7 (da decidere: candidati = SMTP per invitation send, oppure primo modulo prodotto del Marketing OS — Brand Brain o Content Studio iniziale, oppure UI revoke + re-invite)
...
- **Sessione 5 — Admin clients endpoints + onboarding wizard** (commit `1bc0dec` feat backend admin, `a894728` feat frontend admin, `0d89a08` ADR-0006 + JOURNAL + TODO, chiusa il 2026-05-02): admin clients onboarding flow operativo end-to-end. Super_admin può creare nuovi client + invitations via `POST /api/v1/admin/clients` (con `generate_invitation_token` `secrets.token_urlsafe(32)` + `token_hash` SHA-256 in DB, `expires_at` = `now() + 7d`). Frontend route group `(dashboard)/admin/` con role-aware sidebar (sezione "Admin · Clienti" condizionale a `user.role === "super_admin"`), success panel post-create con `invitation_url` copiabile via `navigator.clipboard.writeText` — **mai redirect automatico**, perché il token plaintext è in chiaro SOLO nel response 201 (in DB c'è solo SHA-256). **Schema**: nuova tabella `invitations` (UNIQUE PARTIAL INDEX su `(client_id, email) WHERE accepted_at IS NULL AND revoked_at IS NULL` per re-invite logic, CHECK `email = lower(email)` + CHECK `role IN ('client_admin','client_member')` defense-in-depth) + RLS policies in `supabase/policies/005_invitations.sql` (S5 baseline super_admin only). **Test paranoici 9/9 pass via `scripts/smoke_test_session5.py`** (primo file di test persistente del progetto, pattern da continuare in S6+ — sostituisce gli ad-hoc smoke test inline) — incluso forged token con role manomesso (OPTION A strict checks da S3 funziona end-to-end), RLS direct INSERT block, defense-in-depth a 3 layer (`require_super_admin` dependency → OPTION A token-vs-DB strict checks → RLS WITH CHECK clauses). **41/41 sub-asserzioni E2E** (backend smoke + frontend a-k incluso copy button presence check via static analysis del bundle JS + cleanup CASCADE). **ADR-0006** documenta scope limitations (1 email = 1 user — multi-tenant per utente è Phase 2; email send skip per S5 → TODO SMTP/Resend S6+; token plaintext SOLO in response 201, mai log) + pattern emergente "**Admin Route Family `/api/v1/admin/*`**" simmetrica a `/api/v1/auth/*` + trigger conditions per `react-hook-form`/Zod (>5 input, wizard multi-step) e TanStack Query (mutation client-side complesse — useActionState basta per S5/S6). Side effect: dev port standardization (chore commit `b848800`: backend `:8001`, frontend `:3001` — entrambe `:3000` e `:8000` conflittano con altri progetti dell'utente sulla stessa macchina).
- **Sessione 6 — Accept invitation flow + first client_admin onboarding** (commit `c642dc2` feat backend, `f64b312` feat frontend, `6968b9f` ADR-0007 + JOURNAL + TODO, chiusa il 2026-05-03): chiusura del cerchio onboarding. Un client_admin riceve URL invitation → `/accept-invite` preview valida token → form password con counter live + confirm → `POST /api/v1/auth/accept-invite` single transaction (validate + INSERT user + UPDATE invitation + linkage `accepted_by_user_id`) → tokens generati → cookies httpOnly settati → redirect `/dashboard` (auto-login Opzione A). Backend: GET preview **always-404-on-invalid** (no information disclosure verificato empiricamente con byte-equality su 4 stati distinti), POST 410 sub-cases differenziati al submit (UX vs security calibrato). Frontend: pagina standalone `/accept-invite` (no dashboard layout, pre-auth), 4 fail-paths convergenti in `InvitationErrorState` con messaggio generico + link `/login`, `redirect("/dashboard")` FUORI dal try/catch (NEXT_REDIRECT signal). Schema: migration 0003 add `accepted_by_user_id UUID NULL FK users ON DELETE SET NULL`. **9/9 paranoid checks persistenti via `scripts/smoke_test_session6.py`** (37 sub-asserzioni, idempotente verificato empiricamente). Non-regressione: `smoke_test_session5.py` continua a passare 9/9 dopo S6. **ADR-0007** documenta 8 decisioni: `validate_invitation` centralization, password NIST 800-63B aligned (12+ char, no regex compositi), GET always-404-on-invalid, POST 410 differenziato, **READ COMMITTED + UNIQUE catch (NON SERIALIZABLE — lezione architetturale di S6 step 0**: la proposta SERIALIZABLE iniziale era over-engineering, UNIQUE catch + 409 era già pattern in `admin.py::create_client`; salvata in memoria del workflow), auto-login Opzione A, pattern emergente "**Public Transactional Handler**" (`get_unauthenticated_db` + `session.begin()` esplicito, refactor in helper deferred a ≥2 consumer), `_build_token_pair` shared helper (2 consumer, refactor a ≥3). Pattern stabiliti S6: `smoke_test_sessionN.py` persistente cementato come pattern progetto (3 file ora), schema extraction trigger ≥5 (auth router migrato), `validate_<entity>` con eccezioni dedicate riusabile (candidati S7+: `validate_password_reset_token`, `validate_magic_link`). 27 source files mypy + 7 ADR totali a 6 sessioni.

**Prossima milestone:** Sessione 7 da scope-decidere: priorità candidate = (a) SMTP/SES/Resend per email invitation send (chiude debt UX di onboarding manuale), (b) primo modulo prodotto Marketing OS (Brand Brain o Content Studio iniziale, valore demo per i 3 client interni), (c) UI super_admin per revoke + re-invite invitation (chiude debt operativo).
